...
export const MisEventos = async (elementoPadre, modoAdmin = false) => {
  ...
  try {
    const response = await fetchData({
      url: 'https://proyecto10-full-stack-js-gwfa.vercel.app/api/v1/eventos/eventos-creados',
      method: 'GET',
    ...
    if (response && response.length > 0) {
      eventosCount.textContent = `${response.length} evento${
        response.length !== 1 ? 's' : ''
      }`
...
const crearEventoCard = (evento, elementoPadre) => {
  ...
  btnEdit.addEventListener('click', () => {
    ...
    if (!eventoId) {
      alert(
        'Error: No se puede editar este evento porque no tiene un ID válido'
      )
      return
    }
  ...
  btnDelete.addEventListener('click', () => {
    ...
    if (!eventoId) {
      alert(
        'Error: No se puede eliminar este evento porque no tiene un ID válido'
      )
      return
    }
