...
export const ListaAsistentes = async() => {
        ...
        try {
          ...
          for (const evento of eventosConAsistentes) {
            const asistentesRes = await fetch(`http://localhost:3000/api/v1/eventos/${evento._id}/asistentes`, {
              headers: {
                "Authorization": `Bearer ${token}`
              }
            })
      ...
      // Función auxiliar para añadir o eliminar asistentes (para uso futuro)
      /*export const toggleAsistencia = async (idEvento, idUser, añadir = true) => {
        const token = localStorage.getItem("token")
        
        if (!token) return false
        
        try {
          const method = añadir ? "PUT" : "DELETE"
          const endpoint = añadir 
            ? `http://localhost:3000/api/v1/eventos/${idEvento}/asistentes` 
            : `http://localhost:3000/api/v1/eventos/${idEvento}/asistentes/${idUser}`
          ...
          const options = {
            ...
            body: añadir ? JSON.stringify({ asistente: idUser }) : undefined
          }
