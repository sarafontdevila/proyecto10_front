...
export const ListaAsistentes = async () => {
  ...
  try {
    ...
    const eventosWithFullAsistentes = await Promise.all(eventosConAsistentes.map(async (evento) => {
      let asistentesData = [];
      try {
        ...
        console.error(`Error al obtener asistentes para el evento ${evento._id}:`, e);
      
      }
      return { ...evento, asistentesFullData: asistentesData };
    }));
    ...
    mostrarMensaje("Error al cargar las listas de asistentes")
  }
}